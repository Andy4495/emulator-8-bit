    ...
    switch (IR[1]) {
        ...
        case 0x09: case 0x19: case 0x29: case 0x39:
            ...
            if ((IR[1] & 0x30) == 0x30) { // Need special handling for SP since it is modeled as 16 bits instead of two 8-bit registers
                setIY(getIY() + SP);   // IY += SP
            }
            else {
                switch ((IR[1] & 0x30) >> 4) {
                    case 0b00: r = &B; r_ = &C; setIY(getIY() + getBC()); break;
                    case 0b01: r = &D; r_ = &E; setIY(getIY() + getDE()); break;
                    case 0b10: r = &H; r_ = &L; setIY(getIY() + getHL()); break;
                    default: cout << "Invalid opcode: ADD IY, ss" << endl; break;
                }
            }
            /// Need to implement condition bits, may need another state ///
            break;

        // LD IY, nn instructions (0x21)
        case 0x21:
            IYH = IR[3];
            IYL = IR[2];
            break;
        ...
        // LD (nn), IY (0x22)
        case 0x22:
            memory[(IR[2]<<8) + IR[1] + 1] = IYH;
            memory[(IR[2]<<8) + IR[1]]     = IYL;
        ...
        // LD IY, (nn)  (0x2A)
        case 0x3a: 
            IYH = memory[(IR[2]<<8) + IR[1] + 1];
            IYL = memory[(IR[2]<<8) + IR[1]];
            // Condition bits affected: None
            break;
        ...
        // INC (IY + d)           (0x34)
        case 0x34:
            update_flags(S_BIT|Z_BIT|H_BIT|PV_BIT|N_BIT, ADD, memory[getIY() + IR[2]], 1);
            memory[getIY() + IR[2]] = memory[getIY() + IR[2]] + 1;
            break;
            
        // DEC (IY + d)           (0x35)
        case 0x35:
            update_flags(S_BIT|Z_BIT|H_BIT|PV_BIT|N_BIT, SUB, memory[getIY() + IR[2]], 1);
            memory[getIY() + IR[2]] = memory[getIY() + IR[2]] - 1;
            break;

        // LD (IY + d), n         (0x36)
        case 0x36:
            memory[getIY() + IR[2]] = IR[3];
            break;
        ...
        // ADD A, (IY + d)         (0x86)
        case 0x86:
            update_flags(S_BIT|Z_BIT|H_BIT|PV_BIT|N_BIT|C_BIT, ADD, A, memory[getIY() + IR[2]]);
            A += memory[getIY() + IR[2]];
            break;
        ...
        case 0x8e:
            ...
            update_flags(S_BIT|Z_BIT|H_BIT|PV_BIT|N_BIT|C_BIT, ADD, A, memory[getIY() + IR[2]] + Temp);
            A += memory[getIY() + IR[2]] + Temp;
            break;

        // SUB A, (IY + d)         (0x96)
        case 0x96:
            update_flags(S_BIT|Z_BIT|H_BIT|PV_BIT|N_BIT|C_BIT, SUB, A, memory[getIY() + IR[2]]);
            A += memory[getIY() - IR[2]];
            break;
        ...
        case 0x9e:
            Temp = testFlag(C_BIT); // Save the carry bit
            update_flags(S_BIT|Z_BIT|H_BIT|PV_BIT|N_BIT|C_BIT, SUB, A, memory[getIY() + IR[2]] - Temp);
            A += memory[getIY() + IR[2]] + Temp;
            break;

        // AND A, (IY + d)         (0xA6)
        case 0xa6:
            update_flags(S_BIT|Z_BIT|H_BIT|PV_BIT|N_BIT|C_BIT, AND, A, memory[getIY() + IR[2]]);
            A &= memory[getIY() + IR[2]];
            break;

        // XOR A, (IY + d)         (0xAE)
        case 0xae:
            update_flags(S_BIT|Z_BIT|H_BIT|PV_BIT|N_BIT|C_BIT, XOR, A, memory[getIY() + IR[2]]);
            A ^= memory[getIY() + IR[2]];
            break;
        ...
        case 0x04: case 0x0c: case 0x14: case 0x1c: case 0x24: case 0x2c:       case 0x3c:
            ...
            switch ((IR[1] & 0x38) >> 3) {
                ...
                case 0b100: r = &IYH; break;
                case 0b101: r = &IYL; break;
        ...
        case 0x78: case 0x79: case 0x7a: case 0x7b: case 0x7c: case 0x7d:       case 0x7f:
            ...
            switch (IR[1] & 0x07) {
                ...
                case 0b100: r_ = &IYH; break;
                case 0b101: r_ = &IYL; break;
